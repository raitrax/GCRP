...
Citizen.CreateThread(function()
	...
	for _,tabl in pairs(getCameras()) do
		if(tabl["n"] ~= nil) then
			camerasc[#camerasc+1] = {n=tabl["n"],x=tabl["x"],y=tabl["y"],z=tabl["z"],a=tabl["a"]} 
		end
	end
	...
	while true do
		...
		if (isNearPC() and PlayerData.job ~= nil and PlayerData.job.name == 'police') then
			ShowInfo("Appuyez sur ~g~F~w~ pour allumer l'ordinateur de Sandy Shores.")
		...
		if(lookCamera) then
			...
			if(IsControlJustPressed(1, 175)) then -- Right
				if(currentCamera+1 <= #camerasc) then
					currentCamera = currentCamera+1
					changeCamera()
				else
					currentCamera = 1
			...
			SetTextOutline()
			SetTextEntry("STRING")
			AddTextComponentString("Caméra : "..camerasc[currentCamera].n)
			DrawText(0.45, 0.90)
...
function changeCamera()
	SetCamCoord(cam, camerasc[currentCamera].x, camerasc[currentCamera].y, camerasc[currentCamera].z)
	SetCamRot(cam, 0.0, 0.0, camerasc[currentCamera].a)
...
function isNearPC()
	...
	local distance = GetDistanceBetweenCoords(1853.328, 3689.753, 34.267, coords.x, coords.y, coords.z, true)
...
RegisterNetEvent("camerasc:isCop")
AddEventHandler("camerasc:isCop", function(b)
	isLSPD = b
end)