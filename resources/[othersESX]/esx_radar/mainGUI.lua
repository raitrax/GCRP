...
GUI.optionRect = {0, 0, 0, 190}
GUI.scroller = {132, 132, 132, 255}
...
function Menu.Title(title)
	...
	GUI.Rect(GUI.titleRect, {menuX, 0.1175}, {0.29, 0.085})
end
...
function Menu.Option(option)
	...
	if(currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions) then
		GUI.Text(option, GUI.optionText, {menuX - 0.14, optionCount * 0.035 + 0.125},  {0.5, 0.5 }, false)
		GUI.Rect(GUI.optionRect, { menuX, optionCount * 0.035 + 0.1415 }, { 0.29, 0.035 })
		if(thisOption) then
			GUI.Rect(GUI.scroller, { menuX, optionCount * 0.035 + 0.1415 }, { 0.29, 0.035 })
		end
	elseif (optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption) then
		...
		GUI.Rect(GUI.optionRect, { menuX, optionCount - (currentOption - GUI.maxVisOptions) * 0.035+0.1415 }, { 0.29, 0.035 });
		if(thisOption) then
			GUI.Rect(GUI.scroller, { menuX, optionCount - (currentOption - maxVisOptions) * 0.035 + 0.1415 }, { 0.29, 0.035 })
		end
	end
...
function Menu.Int5(option, int, min, max, cb)
	Menu.Option(option);

	if (optionCount == currentOption) then
		if (leftPressed) then
			if(int > min) then int = int - 5 else int = max end-- : _int = max;
		end
		if (rightPressed) then
			if(int < max) then int = int + 5 else int = min end
		end
	end
...
RegisterNetEvent("radarGUI:Title")
AddEventHandler("radarGUI:Title", function(title)
	Menu.Title(title)
end)

RegisterNetEvent("radarGUI:Option")
AddEventHandler("radarGUI:Option", function(option, cb)
	cb(Menu.Option(option))
end)

RegisterNetEvent("radarGUI:Bool")
AddEventHandler("radarGUI:Bool", function(option, bool, cb)
	Menu.Bool(option, bool, function(data)
		cb(data)
	end)
end)

RegisterNetEvent("radarGUI:Int")
AddEventHandler("radarGUI:Int", function(option, int, min, max, cb)
	Menu.Int(option, int, min, max, function(data)
		cb(data)
	end)
end)

RegisterNetEvent("radarGUI:Int5")
AddEventHandler("radarGUI:Int5", function(option, int, min, max, cb)
	Menu.Int5(option, int, min, max, function(data)
		cb(data)
	end)
end)


RegisterNetEvent("radarGUI:StringArray")
AddEventHandler("radarGUI:StringArray", function(option, array, position, cb)
	Menu.StringArray(option, array, position, function(data)
		cb(data)
	end)
end)

RegisterNetEvent("radarGUI:Update")
AddEventHandler("radarGUI:Update", function()
