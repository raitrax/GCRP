...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('chocolate_clothes_civil'), value = 'citizen_wear'},
				{label = _U('chocolate_clothes_chocolate'), value = 'donpa-choco_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'donpa-choco_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'donpa-choco_actions_menu'
...
function Opendonpa-chocoActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'donpa-choco_actions',
		{
			title    = 'donpa-choco',
			align    = 'top-left',
			elements = elements
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'donpa-choco', function(data, menu)
					menu.close()
				end)
			end
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'donpa-choco', vehicleProps)
...
function OpenMobiledonpa-chocoActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_donpa-choco_actions',
		{
			title    = 'donpa-choco',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_donpa-choco', 'chocolatier', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_chocolatierjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_chocolatierjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_chocolatierjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_chocolatierjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_chocolatierjob:hasEnteredMarker', function(zone)
	if zone == 'féve-de-cacaoFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco'  then
		CurrentAction     = 'féve-de-cacao_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'Traitementchocolat' and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco'  then
		CurrentAction     = 'chocolate_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'Traitementchoco' and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco'  then
		CurrentAction     = 'choco_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'chocolatierActions' and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco' then
		CurrentAction     = 'donpa-choco_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco' then
...
AddEventHandler('esx_chocolatierjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'féve-de-cacaoFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco' then
		TriggerServerEvent('esx_chocolatierjob:stopHarvest')
	end  
	if (zone == 'Traitementchocolat' or zone == 'Traitementchoco') and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco' then
		TriggerServerEvent('esx_chocolatierjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco' then
		TriggerServerEvent('esx_chocolatierjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.chocolatierActions.Pos.x, Config.Zones.chocolatierActions.Pos.y, Config.Zones.chocolatierActions.Pos.z)
		...
		AddTextComponentString("chocolatiers")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_chocolatierjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_chocolatierjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'féve-de-cacao_harvest' then
					TriggerServerEvent('esx_chocolatierjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'choco_traitement' then
					TriggerServerEvent('esx_chocolatierjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'chocolate_traitement' then
					TriggerServerEvent('esx_chocolatierjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_chocolatierjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'donpa-choco_actions_menu' then
					Opendonpa-chocoActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'donpa-choco', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'donpa-choco' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobiledonpa-chocoActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)