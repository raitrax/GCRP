...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    ...
    if PlayerData.job.name == 'conveyor' then
...
AddEventHandler('esx_conveyor:hasEnteredMarker', function(zone)

	if zone == 'CloakRoom' and PlayerData.job.name ~= nil and PlayerData.job.name == 'conveyor' then
		CurrentAction = 'cloakroom_menu'
		CurrentActionMsg = _U('cloakroom')
		CurrentActionData = {}
	end

	if zone == 'VehicleSpawner' and PlayerData.job.name ~= nil and PlayerData.job.name == 'conveyor' then
		CurrentAction = 'vehicle_spawner_menu'
		CurrentActionMsg = _U('veh_spawn')
		CurrentActionData = {}
	end

	if zone == namezone and PlayerData.job.name ~= nil and PlayerData.job.name == 'conveyor' then
			if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) and IsVehicleModel(GetVehiclePedIsUsing(GetPlayerPed(-1)), GetHashKey("Stockade", _r)) then
	...
	if zone == 'RetourCamion' and PlayerData.job.name ~= nil and PlayerData.job.name == 'conveyor' then
...
function OpenVehicleSpawnerMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'conveyor_actions',
		{
...
AddEventHandler('esx_conveyor:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
		...
		AddTextComponentString(_U('Brinks'))
	...
	while true do
		Wait(0)
			...
			if (isInMarker and not hasAlreadyEnteredMarker) or (isInMarker and lastZone ~= currentZone) then
				...
				TriggerEvent('esx_conveyor:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('esx_conveyor:hasExitedMarker', lastZone)
			end
	end
end)
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'cloakroom_menu' and PlayerData.job.name == 'conveyor' then
					OpenCloakRoom()
				end

				if CurrentAction == 'vehicle_spawner_menu' and PlayerData.job.name == 'conveyor' then
					OpenVehicleSpawnerMenu()
				end

				if CurrentAction == 'delivery_ok' and PlayerData.job.name == 'conveyor' then
					nouvelledestination()
				end

				if CurrentAction == 'vehicle_delete_menu' and PlayerData.job.name == 'conveyor' then
					if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) and IsVehicleModel(GetVehiclePedIsUsing(GetPlayerPed(-1)), GetHashKey("Stockade", _r)) then
...
function donnerlapaye()
	...
	if vievehicule >= 1 then
		if livraisonTotalPaye == 0 then
			...
			TriggerServerEvent("esx_conveyor:pay", amount)
...
function MissionLivraisonLetsGo()
	...
	if MissionRegion == 1 then -- Los santos
		TriggerEvent('esx:showNotification', 'Des fonds sont à retirer à Los Santos')
	elseif MissionRegion == 2 then -- Blaine County
		TriggerEvent('esx:showNotification', 'Des fonds sont à retirer Blaine County')
	elseif MissionRegion == 0 then -- au cas ou
		TriggerEvent('esx:showNotification', 'Des fonds sont à retirer ')
	end
...
function MissionLivraisonStopRetourDepot()
	...
	TriggerEvent('esx:showNotification', 'Retournez au dépôt pour finir votre tournée.')
