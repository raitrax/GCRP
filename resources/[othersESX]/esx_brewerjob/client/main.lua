  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'cloakroom',
    {
      ...
      elements = {
        ...
        {label = _U('ems_clothes_ems'), value = 'brewer_wear'},
      },
    },
    function(data, menu)
      ...
      if data.current.value == 'brewer_wear' then
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
      ...
      CurrentAction     = 'brewer_actions_menu'
...
function OpenbrewerActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'brewer_actions',
    {
      title    = 'brewer',
      elements = elements
    },
	...
	if data.current.value == 'cloakroom' then
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('esx_society:openBossMenu', 'brewer', function(data, menu)
          menu.close()
        end)
      end
...
function OpenVehicleSpawnerMenu()
  ...
  if Config.EnableSocietyOwnedVehicles then
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'brewer', vehicleProps)
	...
	local elements = {
			 {label = 'Voiture de Travail',  value = 'rocoto'},
			 {label = 'Camionnette de Travail',  value = 'Speedo'},
	}
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
		...
		ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 309.530, function(vehicle)
			local playerPed = GetPlayerPed(-1)
			TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
      local plate = GetVehicleNumberPlateText(vehicle)
      TriggerServerEvent('esx_vehiclelock:givekey', 'no', plate)
...
function OpenMobilebrewerActionsMenu()

  ESX.UI.Menu.CloseAll()

  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'mobile_brewer_actions',
    {
      title    = 'brewer',
      elements = {
          ...
          function(data, menu)
            ...
            if amount == nil or amount <= 0 then
			  ...
			  local playerPed        = GetPlayerPed(-1)
                ...
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_brewer', 'brewer', amount)
				end)
              end
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_brewerjob:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil or count <= 0 then
              ...
              TriggerServerEvent('esx_brewerjob:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_brewerjob:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil or count <= 0 then
              ...
              TriggerServerEvent('esx_brewerjob:putStockItems', itemName, count)
            end
...
AddEventHandler('esx_brewerjob:hasEnteredMarker', function(zone)
		if zone == 'IngredientsBeer' and PlayerData.job ~= nil and PlayerData.job.name == 'brewer'  then
                CurrentAction     = 'IngredientsBeer_harvest'
                CurrentActionMsg  = _U('press_collect')
                CurrentActionData = {zone= zone}
        end
		
		if zone == 'TraitementBeer' and PlayerData.job ~= nil and PlayerData.job.name == 'brewer'  then
				CurrentAction     = 'Beer_traitement'
				CurrentActionMsg  = _U('press_traitement')
				CurrentActionData = {zone= zone}
		end		
		
		if zone == 'SellBeer'and PlayerData.job ~= nil and PlayerData.job.name == 'brewer'  then
				CurrentAction     = 'Beer_resell'
                CurrentActionMsg  = _U('press_sell')
                CurrentActionData = {zone = zone}
        end
		if zone == 'brewerActions' then
				CurrentAction     = 'brewer_actions_menu'
				CurrentActionMsg  = _U('press_to_open')
				CurrentActionData = {}
		end
  
		if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'brewer' then
			CurrentAction     = 'vehicle_spawner_menu'
			CurrentActionMsg  = _U('spawn_veh')
			CurrentActionData = {}
		end
		
		if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'brewer' then
...
AddEventHandler('esx_brewerjob:hasExitedMarker', function(zone)
  ESX.UI.Menu.CloseAll()
		if (zone == 'IngredientsBeer') then
			TriggerServerEvent('esx_brewerjob:stopHarvest')
		end  
		if (zone == 'TraitementBeer') then
			TriggerServerEvent('esx_brewerjob:stopTransform')
		end  
		if (zone == 'SellBeer') then
			TriggerServerEvent('esx_brewerjob:stopSell')
		end
		CurrentAction = nil
end)
...
function CreateBlip()
    if PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then   
        ...
        BlipIngredientsBeer = AddBlipForCoord(Config.Zones.IngredientsBeer.Pos.x, Config.Zones.IngredientsBeer.Pos.y, Config.Zones.IngredientsBeer.Pos.z)
        SetBlipSprite(BlipIngredientsBeer, Config.Zones.IngredientsBeer.BlipSprite)
        SetBlipColour(BlipIngredientsBeer, Config.Zones.IngredientsBeer.BlipColor)
        SetBlipAsShortRange(BlipIngredientsBeer, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config.Zones.IngredientsBeer.BlipName)
        EndTextCommandSetBlipName(BlipIngredientsBeer)  

        BlipTraitementBeer = AddBlipForCoord(Config.Zones.TraitementBeer.Pos.x, Config.Zones.TraitementBeer.Pos.y, Config.Zones.TraitementBeer.Pos.z)
        SetBlipSprite(BlipTraitementBeer, Config.Zones.TraitementBeer.BlipSprite)
        SetBlipColour(BlipTraitementBeer, Config.Zones.TraitementBeer.BlipColor)
        SetBlipAsShortRange(BlipTraitementBeer, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config.Zones.TraitementBeer.BlipName)
        EndTextCommandSetBlipName(BlipTraitementBeer) 

        BlipSellBeer = AddBlipForCoord(Config.Zones.SellBeer.Pos.x, Config.Zones.SellBeer.Pos.y, Config.Zones.SellBeer.Pos.z)
        SetBlipSprite(BlipSellBeer, Config.Zones.SellBeer.BlipSprite)
        SetBlipColour(BlipSellBeer, Config.Zones.SellBeer.BlipColor)
        SetBlipAsShortRange(BlipSellBeer, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config.Zones.SellBeer.BlipName)
        EndTextCommandSetBlipName(BlipSellBeer) 

        BlipbrewerActions = AddBlipForCoord(Config.Zones.brewerActions.Pos.x, Config.Zones.brewerActions.Pos.y, Config.Zones.brewerActions.Pos.z)
        SetBlipSprite(BlipbrewerActions, Config.Zones.brewerActions.BlipSprite)
        SetBlipColour(BlipbrewerActions, Config.Zones.brewerActions.BlipColor)
        SetBlipAsShortRange(BlipbrewerActions, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config.Zones.brewerActions.BlipName)
        EndTextCommandSetBlipName(BlipbrewerActions) 
...
Citizen.CreateThread(function()
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'brewer' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'brewer' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_brewerjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_brewerjob:hasExitedMarker', LastZone)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'brewer' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'IngredientsBeer_harvest' then
					TriggerServerEvent('esx_brewerjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'Beer_traitement' then
					TriggerServerEvent('esx_brewerjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'Beer_resell' then
					TriggerServerEvent('esx_brewerjob:startSell', CurrentActionData.zone)
				end
					   
				if CurrentAction == 'brewer_actions_menu' then
					OpenbrewerActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'brewer', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'brewer' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobilebrewerActionsMenu()
			GUI.Time = GetGameTimer()
		end
