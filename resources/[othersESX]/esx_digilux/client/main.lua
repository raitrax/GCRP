  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'cloakroom',
    {
      ...
      elements = {
        ...
        {label = _U('ems_clothes_ems'), value = 'ordinateur_wear'},
      },
    },
    function(data, menu)
      ...
      if data.current.value == 'ordinateur_wear' then
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
      ...
      CurrentAction     = 'ordinateur_actions_menu'
...
function OpenordinateurActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'ordinateur_actions',
    {
      title    = 'ordinateur',
      elements = elements
    },
	...
	if data.current.value == 'cloakroom' then
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('esx_society:openBossMenu', 'ordinateur', function(data, menu)
          menu.close()
        end)
      end
...
function OpenVehicleSpawnerMenu()
  ...
  if Config.EnableSocietyOwnedVehicles then
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'ordinateur', vehicleProps)
	...
	local elements = {
			 {label = 'Véhicule de Travail',  value = 'burrito2'},
	}
	-- if (Config.EnablePlayerManagement and PlayerData.job.grade_name == 'boss') or (Config.EnablePlayerManagement and PlayerData.job.grade_name == 'manager') then -- Config.EnablePlayerManagement and PlayerData.job.grade_name == 'boss'
		-- table.insert(elements, {label = 'Véhicule patron', value = 'burrito3'})
	-- end
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'vehicle_spawner',
      {
      ...
      function(data, menu)
		...
		ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 56.326, function(vehicle)
			-- if data.current.value == 'phantom' then
				-- local veh = CreateVehicle('phantom', Config.Zones.VehicleSpawnPoint.Pos.x, Config.Zones.VehicleSpawnPoint.Pos.y, Config.Zones.VehicleSpawnPoint.Pos.z, 270.0, true, false)
				-- local trailer = CreateVehicle('tanker', Config.Zones.VehicleSpawnPoint.Pos.x, Config.Zones.VehicleSpawnPoint.Pos.y, Config.Zones.VehicleSpawnPoint.Pos.z, 270.0, true, false)
				-- AttachVehicleToTrailer(veh, trailer, 1.1)
			-- end
			-- if model == 'burrito3' then
				-- local veh = CreateVehicle(phantom, Config.Zones.VehicleSpawnPoint.Pos.x, Config.Zones.VehicleSpawnPoint.Pos.y, Config.Zones.VehicleSpawnPoint.Pos.z, 270.0, true, false)
			-- end
			local playerPed = GetPlayerPed(-1)
			TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
...
function OpenMobileordinateurActionsMenu()

  ESX.UI.Menu.CloseAll()

  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'mobile_ordinateur_actions',
    {
      title    = 'ordinateur',
      elements = {
          ...
          function(data, menu)
            ...
            if amount == nil or amount <= 0 then
			  ...
			  local playerPed        = GetPlayerPed(-1)
                ...
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ordinateur', 'Informaticien', amount)
				end)
              end
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_ordinateurjob:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil or count <= 0 then
              ...
              TriggerServerEvent('esx_ordinateurjob:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_ordinateurjob:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil or count <= 0 then
              ...
              TriggerServerEvent('esx_ordinateurjob:putStockItems', itemName, count)
            end
...
AddEventHandler('esx_ordinateurjob:hasEnteredMarker', function(zone)
		if zone == 'composantFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur'  then
                CurrentAction     = 'raisin_harvest'
                CurrentActionMsg  = _U('press_collect')
                CurrentActionData = {zone= zone}
        end
		
		if zone == 'Montage' and PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur'  then
				CurrentAction     = 'vine_traitement'
				CurrentActionMsg  = _U('press_traitement')
				CurrentActionData = {zone= zone}
		end		
		
		--if zone == 'TraitementJus' and PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur'  then
                --CurrentAction     = 'jus_traitement'
                --CurrentActionMsg  = _U('press_traitement')
                --CurrentActionData = {zone = zone}
        --end
		 if zone == 'SellPcComplet'and PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur'  then
				 CurrentAction     = 'vine_resell'
                 CurrentActionMsg  = _U('press_sell')
                 CurrentActionData = {zone = zone}
         end
		 --if zone == 'SellJus'and PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur'  then
				 --CurrentAction     = 'jus_resell'
                 --CurrentActionMsg  = _U('press_sell')
                 --CurrentActionData = {zone = zone}
         --end
--[[
		if zone == 'SellFarm'and PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur'  then
				CurrentAction     = 'farm_resell'
                CurrentActionMsg  = _U('press_sell')
                CurrentActionData = {zone = zone}
        end
--]]
		if zone == 'ordinateurActions' then
				CurrentAction     = 'ordinateur_actions_menu'
				CurrentActionMsg  = _U('press_to_open')
				CurrentActionData = {}
		end
  
		if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur' then
			CurrentAction     = 'vehicle_spawner_menu'
			CurrentActionMsg  = _U('spawn_veh')
			CurrentActionData = {}
		end
		
		if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur' then
...
AddEventHandler('esx_ordinateurjob:hasExitedMarker', function(zone)
  ESX.UI.Menu.CloseAll()
		if (zone == 'composantFarm') then
			TriggerServerEvent('esx_ordinateurjob:stopHarvest')
		end  
		if (zone == 'Montage' or zone == 'TraitementJus') then
			TriggerServerEvent('esx_ordinateurjob:stopTransform')
		end
		--if (zone == 'SellFarm') then
		if (zone == 'SellPcComplet' or zone == 'SellJus') then
			TriggerServerEvent('esx_ordinateurjob:stopSell')
		end
		CurrentAction = nil
end)
...
function CreateBlip()
    if PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then   
        ...
        BlipcomposantFarm = AddBlipForCoord(Config.Zones.composantFarm.Pos.x, Config.Zones.composantFarm.Pos.y, Config.Zones.composantFarm.Pos.z)
        SetBlipSprite(BlipcomposantFarm, Config.Zones.composantFarm.BlipSprite)
        SetBlipColour(BlipcomposantFarm, Config.Zones.composantFarm.BlipColor)
        SetBlipAsShortRange(BlipcomposantFarm, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config.Zones.composantFarm.BlipName)
        EndTextCommandSetBlipName(BlipcomposantFarm)  

        BlipMontage = AddBlipForCoord(Config.Zones.Montage.Pos.x, Config.Zones.Montage.Pos.y, Config.Zones.Montage.Pos.z)
        SetBlipSprite(BlipMontage, Config.Zones.Montage.BlipSprite)
        SetBlipColour(BlipMontage, Config.Zones.Montage.BlipColor)
        SetBlipAsShortRange(BlipMontage, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config.Zones.Montage.BlipName)
        EndTextCommandSetBlipName(BlipMontage) 

        --BlipTraitementJus = AddBlipForCoord(Config.Zones.TraitementJus.Pos.x, Config.Zones.TraitementJus.Pos.y, --Config.Zones.TraitementJus.Pos.z)
        --SetBlipSprite(BlipTraitementJus, Config.Zones.TraitementJus.BlipSprite)
        --SetBlipColour(BlipTraitementJus, Config.Zones.TraitementJus.BlipColor)
        --SetBlipAsShortRange(BlipTraitementJus, true)
        --BeginTextCommandSetBlipName("STRING")
        --AddTextComponentString(Config.Zones.TraitementJus.BlipName)
        --EndTextCommandSetBlipName(BlipTraitementJus) 

        BlipSellPcComplet = AddBlipForCoord(Config.Zones.SellPcComplet.Pos.x, Config.Zones.SellPcComplet.Pos.y, Config.Zones.SellPcComplet.Pos.z)
        SetBlipSprite(BlipSellPcComplet, Config.Zones.SellPcComplet.BlipSprite)
        SetBlipColour(BlipSellPcComplet, Config.Zones.SellPcComplet.BlipColor)
        SetBlipAsShortRange(BlipSellPcComplet, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config.Zones.SellPcComplet.BlipName)
        EndTextCommandSetBlipName(BlipSellPcComplet) 

        --BlipSellJus = AddBlipForCoord(Config.Zones.SellJus.Pos.x, Config.Zones.SellJus.Pos.y, Config.Zones.SellJus.Pos.z)
        --SetBlipSprite(BlipSellJus, Config.Zones.SellJus.BlipSprite)
        --SetBlipColour(BlipSellJus, Config.Zones.SellJus.BlipColor)
        --SetBlipAsShortRange(BlipSellJus, true)
        --BeginTextCommandSetBlipName("STRING")
        --AddTextComponentString(Config.Zones.SellJus.BlipName)
        --EndTextCommandSetBlipName(BlipSellJus) 

        BlipordinateurActions = AddBlipForCoord(Config.Zones.ordinateurActions.Pos.x, Config.Zones.ordinateurActions.Pos.y, Config.Zones.ordinateurActions.Pos.z)
        SetBlipSprite(BlipordinateurActions, Config.Zones.ordinateurActions.BlipSprite)
        SetBlipColour(BlipordinateurActions, Config.Zones.ordinateurActions.BlipColor)
        SetBlipAsShortRange(BlipordinateurActions, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config.Zones.ordinateurActions.BlipName)
        EndTextCommandSetBlipName(BlipordinateurActions) 

        BlipVehicleSpawner = AddBlipForCoord(Config.Zones.VehicleSpawner.Pos.x, Config.Zones.VehicleSpawner.Pos.y, Config.Zones.VehicleSpawner.Pos.z)
        SetBlipSprite(BlipVehicleSpawner, Config.Zones.VehicleSpawner.BlipSprite)
        SetBlipColour(BlipVehicleSpawner, Config.Zones.VehicleSpawner.BlipColor)
        SetBlipAsShortRange(BlipVehicleSpawner, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config.Zones.VehicleSpawner.BlipName)
        EndTextCommandSetBlipName(BlipVehicleSpawner) 
...
local blip = AddBlipForCoord(Config.Zones.ordinateurActions.Pos.x, Config.Zones.ordinateurActions.Pos.y, Config.Zones.ordinateurActions.Pos.z)
  ...
  BeginTextCommandSetBlipName("STRING")
  AddTextComponentString("ordinateurrons")
  EndTextCommandSetBlipName(blip)
  publicBlip = true
  end
    if PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur' then
...
-- Create Blips
-- Citizen.CreateThread(function()
  -- local blip = AddBlipForCoord(Config.Zones.RaffineurActions.Pos.x, Config.Zones.RaffineurActions.Pos.y, Config.Zones.RaffineurActions.Pos.z)
  -- SetBlipSprite (blip, 436)
  -- SetBlipDisplay(blip, 4)
  -- SetBlipScale  (blip, 1.0)
  -- SetBlipColour (blip, 5)--40
  -- SetBlipAsShortRange(blip, true)

  -- BeginTextCommandSetBlipName("STRING")
  -- AddTextComponentString("DOil'")
  -- EndTextCommandSetBlipName(blip)
  
	-- if (PlayerData.job ~= nil and PlayerData.job.name == 'fuel') then
	  -- local blip2 = AddBlipForCoord(Config.Zones.PetrolFarm.Pos.x, Config.Zones.PetrolFarm.Pos.y, Config.Zones.PetrolFarm.Pos.z)
		...
		-- BeginTextCommandSetBlipName("STRING")
		-- AddTextComponentString(Config.Zones.PetrolFarm.Name)
		-- EndTextCommandSetBlipName(blip2)
	-- end
  ...
  -- if (PlayerData.job ~= nil and PlayerData.job.name == 'fuel') then
	  ...
	  -- for i=1, #Config.Zones, 1 do
		-- if(Config.Zones[i] == "RaffineurActions" and Config.Zones[i] == "VehicleSpawnPoint") then
			-- local blip2 = AddBlipForCoord(Config.Zones[i].Pos.x, Config.Zones[i].Pos.y, Config.Zones[i].Pos.z)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_ordinateurjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_ordinateurjob:hasExitedMarker', LastZone)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_ordinateurjob:startHarvest', CurrentActionData.zone)
				end
				--if CurrentAction == 'jus_traitement' then
					--TriggerServerEvent('esx_ordinateurjob:startTransform', CurrentActionData.zone)
				--end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('esx_ordinateurjob:startTransform', CurrentActionData.zone)
				end
				 --if CurrentAction == 'jus_resell' then
					 --TriggerServerEvent('esx_ordinateurjob:startSell', CurrentActionData.zone)
				 --end
				 if CurrentAction == 'vine_resell' then
					 TriggerServerEvent('esx_ordinateurjob:startSell', CurrentActionData.zone)
				 end
				...
				if CurrentAction == 'ordinateur_actions_menu' then
					OpenordinateurActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'ordinateur', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'ordinateur' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileordinateurActionsMenu()
			GUI.Time = GetGameTimer()
		end
