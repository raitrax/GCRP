...
function OpenCloakroomMenu()
    ...
    {label = 'Tenue blindée', value = 'swat_wear'},	
	}
	
	if PlayerData.job ~= nil and (PlayerData.job.grade_name == 'prospect' or PlayerData.job.grade_name == 'petit frappe' or PlayerData.job.grade_name == 'bras droit') then
		table.insert(elements, {label = 'Tenue 1', value = 'fib_wear'})
	end
	
	if PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = 'Tenue patron ', value = 'fibboss_wear'})	
	end
	...
	if Config.EnableNonFreemodePeds then
		...
		ESX.UI.Menu.Open(
			...
			if data.current.value == 'fib_wear' and PlayerData.job.name == 'niker' then
		...
		if data.current.value == 'swat_wear' and PlayerData.job.name == 'biker' then
...
function OpenVehicleSpawnerMenu(station, partNum)
  ...
  if Config.EnableSocietyOwnedVehicles then
    ...
    local elements = {}
	  ...
	  table.insert(elements, { label = 'bmw',      value = 'bs17'})
	  --table.insert(elements, { label = 'aston',    value = 'amv19'})	
      table.insert(elements, { label = 'hexer',    value = 'hexer'})
	  table.insert(elements, { label = 'schafter', value = 'schafter2'})
    ...
    function(data, menu)
      ...
      if data.current.value == 'citizen_interaction' then
        ...
        if data2.current.value == 'code' then
                TriggerServerEvent('esx_bikerjob:codedmv', GetPlayerServerId(player))
              end
        
        if data2.current.value == 'codedrive' then
                TriggerServerEvent('esx_bikerjob:codedrive', GetPlayerServerId(player))
              end
        
        if data2.current.value == 'codedrivebike' then
                TriggerServerEvent('esx_bikerjob:codedrivebike', GetPlayerServerId(player))
              end
        
        if data2.current.value == 'codedrivetruck' then
                TriggerServerEvent('esx_bikerjob:codedrivetruck', GetPlayerServerId(player))
              end 
        
        if data2.current.value == 'weaponlicense' then
                TriggerServerEvent('esx_bikerjob:weaponlicense', GetPlayerServerId(player))
              end
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if Config.EnablePlayerManagement then
          TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_bikerjob', _U('fine_total') .. label, amount)
        else
...
Citizen.CreateThread(function()
  while true do
	...
	if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'biker' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'fib_actions') and (GetGameTimer() - GUI.Time) > 150 then
      OpenFibActionsMenu()
      GUI.Time = GetGameTimer()
    end
