...
RegisterNetEvent('esx_ambulancejob:respawnbutton')
AddEventHandler('esx_ambulancejob:respawnbutton', function()
	DoScreenFadeOut(800)
	...
	ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeathrespawnbutton', function()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				-- {label = 'Tenue pompier', value = 'firefighter_wear'},
				-- {label = 'Tenue civil après pompier', value = 'citizen_wear2'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'firefighter_wear' then
				isinservice = true
				TriggerServerEvent("player:serviceOn", "ambulance")
			...
			if data.current.value == 'citizen_wear2' then
				isinservice = false
				TriggerServerEvent("player:serviceOff", "ambulance")
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, vehicles[partNum].SpawnPoint, 270.0, function(vehicle)
						...
						SetVehicleNumberPlateText(vehicle, "Ambu112" .. platenum)
						TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
		...
		ESX.UI.Menu.Open(
			'default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				elements = {
					...
					{label = 'Camion pompier', value = 'FIRETRUK'},
				},
			},
			function(data, menu)
...
RegisterNetEvent('esx_ambulancejob:killped')
AddEventHandler('esx_ambulancejob:killped', function(source)
	Citizen.Wait(15000)
...
AddEventHandler('esx_ambulancejob:hasEnteredMarker', function(zone)
	...
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'ambulance' and isinservice == true then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('veh_spawn')
		CurrentActionData = {}
	end
	
	if zone == 'Pharmacy' and PlayerData.job ~= nil and PlayerData.job.name == 'ambulance' and isinservice == true then
        CurrentAction     = 'pharmacy'
        CurrentActionMsg  = _U('open_pharmacy')
        CurrentActionData = {}
    end

	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'ambulance' and isinservice == true then
...
AddEventHandler('esx_ambulancejob:hasEnteredEntityZone', function(entity)
  ...
  if PlayerData.job ~= nil and PlayerData.job.name == 'ambulance' and isinservice == true and not IsPedInAnyVehicle(playerPed, false) then
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlPressed(0,  Keys['N9']) and (GetGameTimer() - GUI.Time) > 150 then

      if OnJob then
        StopAmbulanceJob()
      else

        if PlayerData.job ~= nil and PlayerData.job.name == 'ambulance' and isinservice == true then
...
Citizen.CreateThread(function()
    while true do
        ...
        local pourcent = health/maxhealth * 100
        ...
        if(pourcent ==100 and isInjured)then
			isInjured = false
