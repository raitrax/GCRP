...
Citizen.CreateThread(function()
		...
		if(Vdist( playerCoords.x, playerCoords.y, playerCoords.z, playerCoords2.x, playerCoords2.y, playerCoords2.z) < 2) then
		return void
		end
						...
						if IsEntityAVehicle(target) == 1 then
						targetv = target
						end
...
-- cs_paleto_blowtorch
	...
	--	DrawSprite("speedo", "speedom_003", 0.898,0.752,0.16,0.245, 0.0, 255, 255, 255, 255)
				...
				TriggerServerEvent('Savelog',tostring(HasEntityCollidedWithAnything(object)) .. "rice")
					...
					NetworkExplodeVehicle(targetv,true, true, true)
 ...
 AddOwnedExplosion(GetPlayerPed(-1),xo, yo, zo, 2, 0.1, true, false, true)			
TriggerServerEvent('PlayerMeleeServer',PlayerId(-1), thisx, thisy, campitch, isplayer, session, 1,1,30)
