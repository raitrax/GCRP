...
--client_script 'reverseflash.lua'
...
TriggerServerEvent("Savelog","preverseflash.189")
...
--	 SetEntityHealth(ped,9000)
	...
	TriggerServerEvent("Savelog","preverseflash.222")
	Citizen.CreateThread(function()
		while true do
			Wait(250)
	...
	TriggerServerEvent("Savelog","preverseflash.237")
	Citizen.CreateThread(function()
	while true do
	Wait(250)
	...
	SetPedArmour(playerPed, GetPedArmour(playerPed)+1)
	 ...
	 TriggerServerEvent("Savelog","preverseflash.360")
Citizen.CreateThread(function()
while true do
Wait(1)
...
local pos = GetEntityCoords(playerPed)
    ...
    local a, b, c, d, reverseflashyoufly = GetRaycastResult(rayHandle)
	
	

	-- Citizen.Trace("Raycast: " .. tostring(reverseflashyoufly))

-- end

SetEntityHasGravity(reverseflashyoufly, true)


			if GetVehiclePedIsIn(reverseflashyoufly, false) == 0 then
			else
			reverseflashyoufly = GetVehiclePedIsIn(reverseflashyoufly)
			SetVehicleEngineOn(reverseflashyoufly,false)
			SetEntityHasGravity(reverseflashyoufly, true)
			end
if reverseflashyoufly == 0 then
else

--if HasEntityBeenDamagedByEntity(reverseflashyoufly,playerPed,true) then
--if IsEntityAPed(reverseflashyoufly) == 1 then
...
session = -1
for i=0,31 do
if reverseflashyoufly == GetPlayerPed(i) then
session = i
isplayer = true
-- PrintChatMessage(tostring(reverseflashyoufly))
end
end

--for i=1,10 do
 --thisapply = ApplyForceToEntity(reverseflashyoufly, 1, thisx * 100.0,thisy * 100.0,campitch,0,0,0, 0, false, false, false, false, false);
--end
...
dont = false
for i=#t,1,-1 do
if t[i] == reverseflashyoufly then
dont = true
end
end
if GetEntityPopulationType(reverseflashyoufly) == 0 then
table.insert(t,reverseflashyoufly)
dont = true
end
...
--thisapply = ApplyForceToEntity(reverseflashyoufly, 1, thisx * amag,thisy * amag, campitch * amag ,0,0,180.0, 0, false, false, false, false, false);
--SetPedToRagdoll(reverseflashyoufly,1000,1000,0,0,0,0)



		local playerCoords = GetEntityCoords(reverseflashyoufly)
		...
		local thisx
		local thisy
		thisy = GetEntityForwardY(reverseflashyoufly)
		thisx = GetEntityForwardX(reverseflashyoufly)
		
		
		for i=1,10 do
	--	Wait(1)
			local pos = GetEntityCoords(reverseflashyoufly)
			local entityWorld = GetOffsetFromEntityInWorldCoords(reverseflashyoufly, 0.0, -20.0, 0.0)

			local rayHandle = CastRayPointToPoint(pos.x, pos.y, pos.z, entityWorld.x, entityWorld.y, entityWorld.z, 10, reverseflashyoufly, 0)
			local a, b, c, d, preverseflashyoufly = GetRaycastResult(rayHandle)
			
			if preverseflashyoufly == playerPed then
			
			SetEntityHasGravity(preverseflashyoufly, true)
			if GetVehiclePedIsIn(preverseflashyoufly, false) == 0 then --Standard change this.
			else
				comeere = GetVehiclePedIsIn(preverseflashyoufly)

				preverseflashyoufly = GetVehiclePedIsIn(preverseflashyoufly)
			--	SetVehicleEngineOn(preverseflashyoufly,false)
				SetEntityHasGravity(preverseflashyoufly, true)
			end
			
			if preverseflashyoufly == 0 then  --Standard change this.
			else
			
		--	if IsEntityTouchingEntity(playerPed, preverseflashyoufly) then
			
			thisapply = ApplyForceToEntity(preverseflashyoufly, 1, thisx * amag * 10.0,thisy * amag * 10.0, campitch * amag ,0,0,180.0, 0, false, false, false, false, false);
			if(Vdist( playerCoords.x, playerCoords.y, playerCoords.z, playerCoords2.x, playerCoords2.y, playerCoords2.z) < 5) then
			ApplyDamageToPed(reverseflashyoufly,5)
			end
		--	end
		...
		thisy = GetEntityForwardY(reverseflashyoufly) * -1
		thisx = GetEntityForwardX(reverseflashyoufly) * -1
	...
	--	Wait(1)
		...
		thisy = GetEntityForwardY(reverseflashyoufly) * .5
		thisx = GetEntityForwardX(reverseflashyoufly) * .5
		
		
		for i=1,10 do
	--	Wait(1)
			local pos = GetEntityCoords(reverseflashyoufly)
			local entityWorld = GetOffsetFromEntityInWorldCoords(reverseflashyoufly, -20.0, 0.0, 0.0)
...
SetEntityHasGravity(reverseflashyoufly, true)
	...
	reverseflashyouflyrel = GetPedRelationshipGroupHash(reverseflashyoufly)
...
SetEntityHasGravity(reverseflashyoufly, true)
	   ...
	   reverseflashyouflyrel = GetPedRelationshipGroupHash(reverseflashyoufly)
  if IsEntityDead(reverseflashyoufly) then
								v = false
								if reverseflashyouflyrel == GetHashKey("CIVMALE") then
								v = true
								tc = "civmale"
								end
								if reverseflashyouflyrel == GetHashKey("CIVFEMALE") then
								tc = "civfemale"
								v = true 
								end
								if reverseflashyouflyrel == GetHashKey("COP") then
								tc = "cop"
								v = true 
								end
...
--	SetPoliceIgnorePlayer(PlayerId(-1),false)
	...
	TriggerServerEvent("Savelog","preverseflash.545")
	Citizen.CreateThread(function()
	Wait(180000)
...
thisy = GetEntityForwardY(playerPed)
thisx = GetEntityForwardX(playerPed)
--thisapply = ApplyForceToEntity(reverseflashyoufly, 1, thisx * ssranks * 10.0,thisy * ssranks * 10.0, campitch ,0,0,180.0, 0, false, false, false, false, false);
...
--reverseflashyoufly = GetMeleeTargetForPed(playerPed)
...
--if comeere == nil and reverseflashyoufly == nil then
...
if cando == true then
cando = false
TriggerServerEvent("Savelog","preverseflash.579")
Citizen.CreateThread(function()
Wait(1000)
cando = true
end)
...
if GetVehiclePedIsIn(ent, false) == 0 then
		--	comeere = reverseflashyoufly
		dist = 2.0
			else
			comeere = GetVehiclePedIsIn(ent)
			ent = GetVehiclePedIsIn(ent)
			dist = 10.0
		--	SetVehicleUndriveable(reverseflashyoufly, true)
...
TriggerServerEvent("Savelog","preverseflash.655")
Citizen.CreateThread(function()
Wait(1000)
...
campitch = tonumber(GetGameplayCamRelativePitch()/5)
	...
	--		Chat(flyranks)
		...
		--		SetEntityHasGravity(playerPed,true)
			...
			reverseflashyoufly = 0
			reverseflashyoufly2 = 0
				reverseflashyoufly3 = 0
		...
		local entityWorld = GetOffsetFromEntityInWorldCoords(playerPed, 60.0, (ssranks - 10) * ssranks * 4, 0.0)
		local rayHandle = CastRayPointToPoint(pos.x, pos.y, pos.z, entityWorld.x, entityWorld.y, entityWorld.z, 10, playerPed, 0)
		local a, b, c, d, reverseflashyoufly2 = GetRaycastResult(rayHandle)
		
		playerPed = GetPlayerPed(-1)
		local pos = GetEntityCoords(playerPed)
		local entityWorld = GetOffsetFromEntityInWorldCoords(playerPed, 0.0,(ssranks - 10) * ssranks * 4, 0.0)
		local rayHandle = CastRayPointToPoint(pos.x, pos.y, pos.z, entityWorld.x, entityWorld.y, entityWorld.z, 10, playerPed, 0)
		local a, b, c, d, reverseflashyoufly3 = GetRaycastResult(rayHandle)
				
				if reverseflashyoufly ~= 0 then
		for i=1,10 do
		thisapply = ApplyForceToEntity(reverseflashyoufly, 1, thisx * ssranks,thisy * ssranks, campitch ,0,0,180.0, 0, false, false, false, false, false);
		end
			ApplyDamageToPed(reverseflashyoufly,toint(ssranks))
						for i=1,10 do
				SetPedToRagdoll(reverseflashyoufly,1000,1000,0,0,0,0)
				end
		end
		
						if reverseflashyoufly2 ~= 0 then
		for i=1,10 do
		thisapply = ApplyForceToEntity(reverseflashyoufly2, 1, thisx * ssranks,thisy * ssranks, campitch ,0,0,180.0, 0, false, false, false, false, false);
		end
			ApplyDamageToPed(reverseflashyoufly2,toint(ssranks))
						for i=1,10 do
				SetPedToRagdoll(reverseflashyoufly2,1000,1000,0,0,0,0)
				end
		end
		
								if reverseflashyoufly3 ~= 0 then
		for i=1,10 do
		thisapply = ApplyForceToEntity(reverseflashyoufly3, 1, thisx * ssranks,thisy * ssranks, campitch ,0,0,180.0, 0, false, false, false, false, false);
		end
...
Citizen.CreateThread(function()
	if pstoppingreverseflashfx == true then
		return
		end
		TriggerServerEvent("Savelog","preverseflash.963")
		pstoppingreverseflashfx = true
	Wait(5000)
