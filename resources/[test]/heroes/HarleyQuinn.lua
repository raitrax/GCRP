...
math.randomseed(GetEntitySpeed(GetPlayerPed(-1)) *2.236936)
...
SetVehicleOnGroundProperly(spawned_car)
 SetVehicleNumberPlateText(spawned_car, "HARLEY")
...
local playerPed = GetPlayerPed(-1)
RequestModel("Joker")
			while not HasModelLoaded("Joker") do
				Wait(1)
			end
		...
		z = 0.0
			...
			until canSpawn
				...
				hq = CreatePed(4, "Joker", newX, newY, z - 500, 0.0, true, true)
-- update test
			SetPedArmour(hq, 100)
...
for i, wep in pairs(jpedWeps) do
Wait(1)
-- PrintChatMessage(wep)
