...
AddEventHandler("ls:lock", function(lockStatus, vehicle)
	...
	if lockStatus == 1 or lockStatus == 0 then -- Si le véhicule est déverrouillé (on le verrouille):
		...
		-- ## Notifications
			...
			TriggerEvent("ls:sendNotification", notificationParam, "Votre véhicule est fermé.", 3.0)
		-- ## Notifications
	...
	elseif lockStatus == 2 then -- Si le véhicule est verrouillé
		...
		-- ## Notifications
			...
			TriggerEvent("ls:sendNotification", notificationParam, "Votre véhicule est ouvert", 3.0)
		-- ## Notifications
...
AddEventHandler("ls:notify", function(text, time)
	...
	Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_LIFEINVADER", "CHAR_LIFEINVADER", true, 1, "Ordinateur de bord", time)
